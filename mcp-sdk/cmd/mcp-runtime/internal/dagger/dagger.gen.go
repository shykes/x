...
type StdioOpts struct {
	StdioTool *File // stdio (../../stdio/main.go:10:2)
}

func (r *Client) Stdio(opts ...StdioOpts) *Stdio { // stdio (../../stdio/main.go:7:1)
	q := r.query.Select("stdio")
	for i := len(opts) - 1; i >= 0; i-- {
		// `stdioTool` optional argument
		if !querybuilder.IsZeroValue(opts[i].StdioTool) {
			q = q.Arg("stdioTool", opts[i].StdioTool)
		}
	}
...
type Stdio struct { // stdio (../../stdio/main.go:17:6)
	query *querybuilder.Selection
...
func (r *Stdio) Proxy(ctr *Container) *Service { // stdio (../../stdio/main.go:22:1)
