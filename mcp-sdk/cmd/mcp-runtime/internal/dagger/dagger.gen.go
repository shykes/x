...
// The `StdioID` scalar type represents an identifier for an object of type Stdio.
type StdioID string
...
// Retrieve the binding value, as type Stdio
func (r *Binding) AsStdio() *Stdio {
	q := r.query.Select("asStdio")
...
// Create or update a binding of type Stdio in the environment
func (r *Env) WithStdioInput(name string, value *Stdio, description string) *Env {
	assertNotNil("value", value)
	q := r.query.Select("withStdioInput")
	q = q.Arg("name", name)
	q = q.Arg("value", value)
	q = q.Arg("description", description)
...
// Declare a desired Stdio output to be assigned in the environment
func (r *Env) WithStdioOutput(name string, description string) *Env {
	q := r.query.Select("withStdioOutput")
	q = q.Arg("name", name)
	q = q.Arg("description", description)
...
// Load a Stdio from its ID.
func (r *Client) LoadStdioFromID(id StdioID) *Stdio {
	q := r.query.Select("loadStdioFromID")
	q = q.Arg("id", id)
...
// StdioOpts contains options for Client.Stdio
type StdioOpts struct {
	ProxySource *Directory // stdio (../../stdio/main.go:12:2)
}

func (r *Client) Stdio(opts ...StdioOpts) *Stdio { // stdio (../../stdio/main.go:8:1)
	q := r.query.Select("stdio")
	for i := len(opts) - 1; i >= 0; i-- {
		// `proxySource` optional argument
		if !querybuilder.IsZeroValue(opts[i].ProxySource) {
			q = q.Arg("proxySource", opts[i].ProxySource)
		}
	}
...
type Stdio struct { // stdio (../../stdio/main.go:19:6)
	query *querybuilder.Selection

	id *StdioID
}

func (r *Stdio) WithGraphQLQuery(q *querybuilder.Selection) *Stdio {
	return &Stdio{
		query: q,
	}
}
...
func (r *Stdio) Binary() *File { // stdio (../../stdio/main.go:42:1)
...
// A unique identifier for this Stdio.
func (r *Stdio) ID(ctx context.Context) (StdioID, error) {
	if r.id != nil {
		return *r.id, nil
	}
	q := r.query.Select("id")

	var response StdioID
...
// XXX_GraphQLType is an internal function. It returns the native GraphQL type name
func (r *Stdio) XXX_GraphQLType() string {
	return "Stdio"
}

// XXX_GraphQLIDType is an internal function. It returns the native GraphQL type name for the ID of this object
func (r *Stdio) XXX_GraphQLIDType() string {
	return "StdioID"
}

// XXX_GraphQLID is an internal function. It returns the underlying type ID
func (r *Stdio) XXX_GraphQLID(ctx context.Context) (string, error) {
	id, err := r.ID(ctx)
	if err != nil {
		return "", err
	}
...
func (r *Stdio) MarshalJSON() ([]byte, error) {
	id, err := r.ID(marshalCtx)
	if err != nil {
		return nil, err
	}
...
// StdioProxyOpts contains options for Stdio.Proxy
type StdioProxyOpts struct {

	// Default: 4242
	Port int // stdio (../../stdio/main.go:27:2)
}

// Execute the given container as a stdio server, and expose it as a TCP service
func (r *Stdio) Proxy(ctr *Container, opts ...StdioProxyOpts) *Service { // stdio (../../stdio/main.go:24:1)
...
func Connect(ctx context.Context, opts ...dagger.ClientOpt) (*Client, error) {
	...
	if err := serveModuleDepencencies(ctx, c); err != nil {
		return nil, err
	}
...
func serveModuleDepencencies(ctx context.Context, client *Client) error {
	...
	for _, dep := range dependencies {
		...
		if depKind == ModuleSourceKindGitSource {
			continue
		}

		if err := dep.AsModule().Serve(ctx); err != nil {
			return err
		}
	}
	...
	if sdkSource != "" {
		if err := modSrc.AsModule().Serve(ctx); err != nil {
			return err
		}
	}
