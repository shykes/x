...
func (m Llm) oaiQuery(ctx context.Context) (comp *openai.ChatCompletion, rerr error) {
	...
	key, err := m.OpenAIToken.Plaintext(ctx)
	if err != nil {
		return nil, err
	}
...
func (m Llm) loadHistory(ctx context.Context) []openai.ChatCompletionMessageParamUnion {
	if m.HistoryJSON == "" {
		return nil
	}
	var raw []Message
...
func (m Llm) saveHistory(history []openai.ChatCompletionMessageParamUnion) Llm {
